...
void Graph_Builder::Check_Connectivity(vector<int>& selected_points, int parent_node) {
	...
	int temp_index;
	auto point_used = new bool[selected_points.size()];
...
int Graph_Builder::Search_Further_Point(vector<int> & points_in_current_node, int current_node_index) const {
	...
	auto current_node = graph->Get_Node(current_node_index);
	if (current_node->degree != 1)
		return -1;
	auto last_node = graph->Get_Node(current_node->adjacent[0]);
	for (auto i = 0; i < 2; ++i)
	...
	for (auto itor = points_in_current_node.begin(); itor < points_in_current_node.end(); ++itor) {
		auto temp_center = candidate_points->Get_Point(*itor);
...
void Graph_Builder::Search_Next_Points() {
	auto current_node = graph->Get_Node_Num() - 1;
	...
	if (selected_points.empty()) {
		auto furthur_point = Search_Further_Point(points_in_current_node, current_node);
		if (furthur_point >= 0) {
...
void Graph_Builder::Connecting_End() const {
	...
	int point_index;
	for (auto i = 0; i < graph->Get_Node_Num(); ++i) {
